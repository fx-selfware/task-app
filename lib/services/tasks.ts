...
export async function createTask(db: Db, taskListId: string, input: CreateTaskInput) {
...
export async function updateTask(db: Db, taskListId: string, taskId: string, input: UpdateTaskInput) {
  const task = await db.select().from(tasks).where(and(eq(tasks.id, taskId), eq(tasks.taskListId, taskListId))).get();
  ...
  if (input.status === 'DONE' && task.parentId === null) {
    return await db.transaction(async (tx) => {
      await tx
        ...
        .run();
      return await tx.update(tasks).set(data).where(eq(tasks.id, taskId)).returning().get();
    });
  }
  ...
  if (input.status === 'TODO' && task.parentId !== null) {
    return await db.transaction(async (tx) => {
      const updated = await tx.update(tasks).set(data).where(eq(tasks.id, taskId)).returning().get();
      await tx.update(tasks).set({ status: 'TODO' }).where(eq(tasks.id, task.parentId!)).run();
      return updated;
    });
  }
...
export async function deleteTask(db: Db, taskListId: string, taskId: string): Promise<void> {
  const task = await db.select().from(tasks).where(and(eq(tasks.id, taskId), eq(tasks.taskListId, taskListId))).get();
  if (!task) httpError(404, 'Not found');
...
export async function deleteCompletedTasks(db: Db, taskListId: string): Promise<void> {
  await db.transaction(async (tx) => {
    ...
    if (doneParentIds.length > 0) {
      await tx.update(tasks).set({ parentId: null }).where(inArray(tasks.parentId, doneParentIds)).run();
    }
...
export async function deleteCompletedSubtasks(db: Db, taskListId: string, parentId: string): Promise<void> {
  const parent = await db
    .select()
    .from(tasks)
...
export async function moveTask(db: Db, taskListId: string, taskId: string, newParentId: string | null) {
  return await db.transaction(async (tx) => {
    const task = await tx.select().from(tasks).where(and(eq(tasks.id, taskId), eq(tasks.taskListId, taskListId))).get();
    if (!task) httpError(404, 'Not found');
    ...
    if (newParentId !== null) {
      ...
      const formerParent = await tx
        .select()
        .from(tasks)
        .where(and(eq(tasks.id, task.parentId), eq(tasks.taskListId, taskListId)))
        .get();
