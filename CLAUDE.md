...
npm run test:e2e        # 39 browser scenarios only
...
- **Only the server can sign you out.** `AuthGuard` sends you to `/login` when the session check comes back *refused* (`isRefusal` in `lib/api/client.ts` — 401/403/404) and shows a retry screen for every other failure, and `useMe` leaves retries to the shared policy in `Providers`. A request that never got an answer says nothing about whether the token is good, and a cold launch — phone radio waking, serverless and database starting cold — is exactly when that happens; treating it as a logout dumped people at a login form their password was never the problem with. Don't reintroduce `retry: false` on `useMe`, and don't collapse the two failure kinds back into one branch.
