Feature: Authentication API
  ...
  Scenario: AC-A1 Register with valid data
  ...
  Scenario: AC-A8 Duplicate email returns 409
    Given a user exists with email "dup@example.com" and password "password123" and name "Dup"
    When I POST "/api/auth/register" with body '{"email":"dup@example.com","password":"other","name":"Dup2"}'
    Then the status is 409

  Scenario: AC-A2 Login with valid credentials
  ...
  Scenario: AC-A3 Login with wrong password returns 401
  ...
  Scenario: AC-A3 Login with unknown email returns 401
    When I POST "/api/auth/login" with body '{"email":"nobody@example.com","password":"password123"}'
    Then the status is 401

  Scenario: AC-A4 GET me with valid cookie returns user
  ...
  Scenario: AC-A5 GET me without cookie returns 401
    When I GET "/api/auth/me"
    Then the status is 401

  Scenario: AC-A6 Logout clears cookie
    Given I am logged in as "logout@example.com" with password "password123"
    When I POST "/api/auth/logout" with my cookie
    Then the status is 200

  Scenario: AC-A7 Token cookie has HttpOnly and SameSite=Strict
