Feature: Templates API
  ...
  Scenario: AC-TP1 Create template
  ...
  Scenario: AC-TP2 Get templates returns only own templates
  ...
  Scenario: AC-TP3 Owner gets template with tasks
  ...
  Scenario: AC-SEC3 Non-owner gets 404 for template
    Given I have a template named "Alice Template"
    When user "bob@example.com" GETs that template
    Then the status is 404

  Scenario: AC-TP4 Owner can rename and delete template
  ...
  Scenario: AC-TP5 Template task CRUD
    Given I have a template named "My Template"
  ...
  Scenario: AC-TP6 Apply template to list appends tasks
